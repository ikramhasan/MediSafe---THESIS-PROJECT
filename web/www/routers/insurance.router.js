...
router.post('/api/contracts', async (req, res) => {
  try {
    ...
    if (await InsurancePeer.authenticateUser(username, password)) {
      ...
      res.json({ success: true, prescriptions: contracts });
      return;
    } else {
...
router.post('/api/prescriptions', async (req, res) => {
  ...
  try {
    const { username, password } = req.body.user;
    if (await InsurancePeer.authenticateUser(username, password)) {
      const prescriptions = await InsurancePeer.getPrescriptions(username);
...
router.post('/api/file-claim', async (req, res) => {
  if (typeof req.body.user !== 'object' ||
    ...
    typeof req.body.grant != 'object') {
    res.json({ error: 'Invalid request!' });
    return;
  }

  try {
    const { user, contractUuid, grant } = req.body;
...
router.post('/api/blocks', async (req, res) => {
  try {
    const blocks = await InsurancePeer.getBlocks(20);
    res.json(blocks);
  } catch (e) {
    res.json({ error: 'Error accessing blockchain.' });
  }
