...
export async function prescribe(contract) {
  if (!isReady()) {
    return;
  }
  try {
    console.log(`prescription before ojbect.assign: ${contract}`)
    let c = Object.assign({}, contract, { uuid: uuidV4() });
    console.log(`prescription after object.assign: ${c}`)
    const loginInfo = await invoke('prescribe', c);
    if (!loginInfo
      ^ !!(loginInfo && loginInfo.username && loginInfo.password)) {
      return Object.assign(loginInfo || {}, { uuid: c.uuid });
    } else {
    ...
    throw wrapError(`Error creating prescription: ${e.message}`, e);
  }
}
