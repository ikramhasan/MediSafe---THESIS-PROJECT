...
export async function getPrescriptions(username) {
  if (!isReady()) {
    return;
  }
  try {
    ...
    const prescriptions = await query('prescriptions_ls', { username });
    return prescriptions;
  } catch (e) {
    let errMessage;
    if (username) {
      errMessage = `Error getting prescriptions for user ${username}: ${e.message}`;
    } else {
      errMessage = `Error getting all prescriptions: ${e.message}`;
    }
    throw wrapError(errMessage, e);
  }
}
