...
func prescribe(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("Invalid argument count.")
	}
	...
	prescriptionKey, err := stub.CreateCompositeKey(prefixPrescription, []string{dto.Username, dto.UUID})
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	err = stub.PutState(prescriptionKey, prescriptionAsBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
