...
func listPrescriptions(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	input := struct {
		Username string `json:"username"`
	}{}
	if len(args) == 1 {
	...
	// Filtering by username if required
	if filterByUsername {
		resultsIterator, err = stub.GetStateByPartialCompositeKey(prefixPrescription, []string{input.Username})
	} else {
	...
	for resultsIterator.HasNext() {
		...
		// Fetch the claims, if the the username parameter is specified
		// if len(input.Username) > 0 {
		// 	result.Claims, err = result.Prescription.Claims(stub)
		// 	if err != nil {
		// 		return shim.Error(err.Error())
		// 	}
		// }
